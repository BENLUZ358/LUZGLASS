...
function lgGlassOptions() {
  ...
  Object.keys(LG_SKU_MAP).forEach(function (sku) {
    ...
    if (/גרפיקה|מראה/.test(name)) return;
...
// עובי + סוג → מק"ט. מחזיר null כשאין פריט כזה, כדי שהמסך לא ימציא אחד.
function lgGlassSku(mm, type) {
  const list = lgGlassOptions()[Number(mm)] || [];
  const hit = list.filter(function (x) { return x.type === type; })[0];
