...
const LG_SKU_MAP = {
  ...
  // חלבי (H) — התזת חול על שקוף. אינו סוג זכוכית אלא **עבודת פנים**:
  // הבסיס הוא שקוף, והפריט נושא שם משלו כי הוא נמכר ומתומחר בנפרד.
  // הקידוד נמסר על ידי בן: 8 שקוף מחוסם + חלבי → 8HMH.
  '6HM':'6 מ"מ חלבי מלוטש',   '6HMH':'6 מ"מ חלבי מחוסם',
  '8HM':'8 מ"מ חלבי מלוטש',   '8HMH':'8 מ"מ חלבי מחוסם',
  '10HM':'10 מ"מ חלבי מלוטש', '10HMH':'10 מ"מ חלבי מחוסם',
  '12HM':'12 מ"מ חלבי מלוטש', '12HMH':'12 מ"מ חלבי מחוסם',
  // פיפטה (P)
...
// ─── איזה זכוכית יש לנו, ובאיזה עובי ────────────────────────────────
//
// **נגזר מ-LG_SKU_MAP ולא נכתב פעם שנייה.** הרשימה הזאת היא הפריטים
// שבאמת קיימים בחשבשבת, ולכן היא לא יכולה להציע ללקוח משהו שאין לו
// מק"ט — ולא לשכוח משהו שיש.
//
// שני סינונים, ושניהם כלל של מפעל ולא בחירה של קוד:
//
//   • **כל מקלחון מחוסם.** לכן רק פריטי "מחוסם" נכנסים; המלוטש הוא
//     אותה זכוכית בעיבוד אחר, ולא סוג אחר.
//   • **גרפיקה היא עיבוד, לא סוג.** "שקוף גרפיקה" הוא שקוף שעוד עתיד
//     לעבור טיפול, ולכן הוא לא סוג נפרד בבחירה. מראות אינן מקלחון.
//
// ולכן ייתכן שכמה מק"טים חולקים סוג אחד ונבדלים רק בעיבוד — וזה בדיוק
// למה הבחירה היא סוג ועובי, והמק"ט נגזר מהם.
function lgGlassOptions() {
  const out = {};
  Object.keys(LG_SKU_MAP).forEach(function (sku) {
    const name = LG_SKU_MAP[sku];
    if (!/מחוסם/.test(name)) return;
    // חלבי וגרפיקה הם **עבודות פנים** ולא סוגים. חלבי אפילו נושא שם
    // פריט משלו — "8 מ''מ חלבי מחוסם" — ובלי הסינון הזה הוא הופיע
    // ברשימת הסוגים כאילו הוא חומר, לצד השקוף שהוא עשוי ממנו.
    if (/גרפיקה|חלבי|מראה/.test(name)) return;
    const m = name.match(/^(\d+) מ"מ (.+?) מחוסם$/);
    if (!m) return;
    const mm = Number(m[1]);
    (out[mm] = out[mm] || []).push({ type: m[2], sku: sku, name: name });
  });
  return out;
}
...
// ─── עבודת פנים ─────────────────────────────────────────────────────
//
// גרפיקה וחלבי אינם סוגי זכוכית אלא **מה שנעשה על הזכוכית**, ולכן הם
// אינם ברשימת הסוגים — אבל הם כן משנים את הפריט, כי לכל אחד מק"ט משלו
// והוא מתומחר בנפרד.
//
// שני כללים שונים, ושניהם מהמק"טים עצמם:
//
//   **גרפיקה** נוספת לסוג: שקוף → שקוף גרפיקה, קליר → קליר גרפיקה.
//   **חלבי** מחליף את הסוג — הוא התזת חול על שקוף, ולכן "8 מ''מ חלבי"
//   ולא "8 מ''מ שקוף חלבי". זה גם מה ש-LG_IMPLIES_BASE_GLASS אומר.
//
// ולכן זה חיפוש בשם ולא הרכבה של אותיות: הקידוד אינו אחיד, ומחרוזת
// שנבנית בקוד תמציא מק"ט שאין לו פריט.
// ‏name מחזיר null כשהעבודה אינה חלה על הסוג הזה. בלי זה חלבי הוצע גם
// על גרניט וגם על אסיד — והחזיר את המק"ט של החלבי־על־שקוף, כי השם
// "8 מ''מ חלבי מחוסם" אינו מזכיר את הסוג בכלל. פריט אחד לשני צירופים
// הוא בדיוק סוג הטעות שמגיעה לחיתוך.
const LG_GLASS_WORK = {
  '':        { he: '',       name: function (t) { return t; } },
  // חלבי = התזת חול, והבסיס שלה הוא שקוף. זה גם מה ש-LG_IMPLIES_BASE_GLASS
  // אומר, ולכן הפריט נקרא "חלבי" ולא "שקוף חלבי".
  'chalavi': { he: 'חלבי',   name: function (t) { return t === LG_GLASS_DEFAULT_BASE ? 'חלבי' : null; } },
  // גרפיקה **נוספת** לסוג ואינה מחליפה אותו, ולכן היא קיימת רק היכן
  // שיש פריט: שקוף וקליר בלבד.
  'graphic': { he: 'גרפיקה', name: function (t) { return t + ' גרפיקה'; } },
};

// עובי + סוג + עבודה → מק"ט. מחזיר null כשאין פריט כזה, כדי שהמסך לא
// ימציא אחד — צירוף שאין לו פריט הוא צירוף שאי אפשר להזמין.
function lgGlassSku(mm, type, work) {
  const w = LG_GLASS_WORK[work || ''] || LG_GLASS_WORK[''];
  const base = w.name(type);
  if (!base) return null;
  const want = Number(mm) + ' מ"מ ' + base + ' מחוסם';
  const hit = Object.keys(LG_SKU_MAP).filter(function (k) {
    return LG_SKU_MAP[k] === want;
  })[0];
  return hit || null;
}

// אילו עבודות אפשריות לצירוף הזה. המסך מציע רק את מה שיש לו פריט.
function lgGlassWorks(mm, type) {
  return Object.keys(LG_GLASS_WORK).filter(function (w) {
    return !!lgGlassSku(mm, type, w);
  });
}
