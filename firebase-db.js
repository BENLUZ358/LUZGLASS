...
// והוא מתומחר בנפרד.
//
// הטבלה נושאת את **המק"ט עצמו** ולא מרכיבה אותו משם. השמות במפעל אינם
// עקביים — "8 מ''מ חלבי מחוסם" בלי המילה שקוף, מול "6 מ''מ שקוף חלבי
// מחוסם" איתה — ובנייה של שם בקוד הייתה מפספסת אחד מהשניים ומחזירה
// null על פריט שקיים.
//
// זו גם הרשימה של מה שנעשה **במקלחון**. פריטי גרפיקה קיימים במפעל גם
// בעוביים אחרים ומשמשים את המערכת במקומות אחרים; כאן מופיע רק מה שבן
// אישר למקלחונים.
const LG_GLASS_WORK = {
  '':        { he: '' },
  'chalavi': { he: 'חלבי' },
...
// מה שנעשה במקלחון. **נגזר מהפריטים שקיימים** ולא נכתב ביד: פריט
// שאין לו מק"ט אינו אפשרות, כי אי אפשר להזמין אותו.
//
// ⚠️ פריטי **חלבי אינם ב-LG_SKU_MAP כלל**. הם קיימים בחשבשבת — המנתח
// יודע לקרוא "8 מ''מ חלבי מחוסם" — אבל הקודים שלהם מעולם לא נרשמו כאן.
// עד שיירשמו, חלבי לא יוצע: עדיף בלי אפשרות מאשר עם מק"ט מומצא.
function _lgWorkRows() {
  const rows = [];
  Object.keys(LG_SKU_MAP).forEach(function (sku) {
    const n = LG_SKU_MAP[sku];
    if (!/מחוסם/.test(n) || /מראה/.test(n)) return;
    var m = n.match(/^(\d+) מ"מ (.+?) גרפיקה מחוסם$/);
    if (m) { rows.push({ work: 'graphic', mm: Number(m[1]), type: m[2], sku: sku }); return; }
    m = n.match(/^(\d+) מ"מ (.+?) חלבי מחוסם$/);
    if (m) { rows.push({ work: 'chalavi', mm: Number(m[1]), type: m[2], sku: sku }); return; }
    m = n.match(/^(\d+) מ"מ חלבי מחוסם$/);
    if (m) rows.push({ work: 'chalavi', mm: Number(m[1]), type: LG_GLASS_DEFAULT_BASE, sku: sku });
  });
  return rows;
}
...
function lgGlassSku(mm, type, work) {
  if (!work) {
    const want = Number(mm) + ' מ"מ ' + type + ' מחוסם';
    return Object.keys(LG_SKU_MAP).filter(function (k) {
      return LG_SKU_MAP[k] === want;
    })[0] || null;
  }
  const hit = LG_SHOWER_WORKS.filter(function (r) {
    return r.work === work && r.mm === Number(mm) && r.type === type;
  })[0];
  // גם כשהשורה קיימת, המק"ט חייב להיות פריט אמיתי. שורה שמצביעה לשום
  // מקום היא הזמנה שתיתקע בחשבשבת.
  return (hit && LG_SKU_MAP[hit.sku]) ? hit.sku : null;
}

// אילו עבודות אפשריות לצירוף הזה, במקלחון.
function lgGlassWorks(mm, type) {
  return Object.keys(LG_GLASS_WORK).filter(function (w) {
    return !w || !!lgGlassSku(mm, type, w);
  });
}
