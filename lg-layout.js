...
// ‏ציר הרמוניקה עצמו יושב **על התפר** (inset:0, כמו כל ציר) — זו נקודת
// הציר הווירטואלית שסביבה מסתובב הזוג. הקדחים שנקדחים בפועל, לעומת
// זאת, יושבים 3.6 ס"מ פנימה מהתפר **בכל זכוכית בנפרד** — זה מה שהסמל
// מצייר (‏±hOff ב-engHardware). הקבוע הזה הוא המקור היחיד למספר הזה,
// כדי שקדח מוצהר (מקלחון שהשותפה עוד לא הגיעה) יידע לצייר את עצמו
// באותו מרחק בדיוק שהצומת האמיתי יצייר כשהיא כן תגיע (בן, 2026-09-19).
const LG_HARMONICA_INSET_MM=36;
...
function lgFromPanels(panels,pStates,opts){
  ...
  //
  // דלת בקצה נתלית על משהו — אבל **בפאה שבה הציר שלה יושב, ולא סתם
  // בקצה**. עד היום לדלת היה ציר בפאה אחת בלבד, ושתי הפאות של הקצה היו
  // בכל מקרה אותה פאה — אז "יש דלת בקצה" ו"הציר בקצה" אמרו אותו דבר.
  // דלת שנושאת צירים בשתי פאותיה (אקורדיון) שוברת את זה: הפאה השנייה
  // אינה נתלית על כלום — היא ממתינה לזכוכית שעוד לא נוספה, וסימונה
  // כקיר הוא בדיוק מה ש-lgValidate בודק ומסרב לו. הבדיקה כאן היא רק
  // גאומטריה — "האם הציר המוצהר יושב בפאה הזאת" — ואינה יודעת כלום
  // על מה מותר להרמוניקה להתחבר אליו; זה עדיין רק ב-lgValidate.
  //
  // ‏'both' הוא היוצא מן הכלל: אותה דלת כבר הוצהרה
  // כמתקפלת בשתי פאותיה, וזו ההצהרה שהופכת גם קיר לאפשרי משני הצדדים —
  // התצורה הנדירה שבה ההרמוניקה כן נוגעת בקיר. זו לא החלטה חדשה כאן,
  // אלא קריאה של מה שהכרטיס כבר קבע, בדיוק כמו wallSide.
  const wallOn=(p,side)=>{ const w=(p&&p.wallSide)||'';
    if(w==='both'||w===side) return true;
    if(!p||p.type!=='door') return false;
    if(p.harmonicaSide==='both') return true;
    const canvasSide = side==='right' ? 'left' : 'right';
    return p.hingeSide === canvasSide;
  };
  ...
  const endNeedsWall=(p,inner)=>{
    ...
    // **לוח בודד הוא היוצא מן הכלל היחיד.** אין לו שכן פנימי, ולכן הצד
    // השני מכריע והוא נשאר עם שתי זוויות — מה שצריך מי שמזמין לוח
    // להחלפה. כל קבוע אחר בקצה ריצה נוגע בקיר, אחרת הוא יוצא בלי שום
    // זווית משלו ואי אפשר להרכיב אותו.
    return !!inner;
  };
  ...
  return {
    ...
    // תוספות שהלקוח מבקש על ההרכבה כולה ואין עליהן חוקיות:
    // מוט חיזוק ומעטפת שחורה. הן עוברות כמו שהן כדי שהליקוט יראה אותן;
    // המנוע אינו מחליט עליהן ואינו מציע אותן.
    supportBar: o.supportBar||null,
    blackTrim:  o.blackTrim ||null,
    // הגומיות — הכנה בלבד. החוקיות שלהן עוד לא נקבעה, ולכן אין כאן
    // ברירת מחדל — מה שנמסר עובר, ומה שלא נמסר נשאר ריק.
    seals: o.seals||null,
    shapes: list.map((p,i)=>{
...
function _layoutPass(shower,cW,mgL,mgR){
  ...
  // גובה ה**ראש** של הלוח בפאה הזאת — לא גובה הפאה.
  //
  // ההבדל קריטי בשיפוע: כששיפוע יושב ב**תחתית**, הראש ישר לכל רוחב
  // הלוח ושתי המידות נבדלות למטה בלבד. השוואה לגובה הפאה שלחה קבוע של
  // 2000/1800 להיקרא כ-1800 מול דלת של 1985 — הפרש 185, כלומר המקרה
  // החריג — והראש של הדלת יצא חמישה מ"מ מעל, והציר נקרא 195 ו-200.
  // רק שיפוע ב**ראש** באמת מוריד את הראש באותה פאה.
  const faceMM=(i,side)=>{
    const sl=slopes[i];
    if(!(sl&&sl.hSide)) return shapeMM(shapes[i],i);
    if(sl.hSide!=='top') return shapeMM(shapes[i],i);   // הראש ישר
