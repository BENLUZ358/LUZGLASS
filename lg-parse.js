...
// 2000 → "2000"  /  1985 → "1985"  /  250 → "0.25"
//
// מה שחוזר לשדה עריכה, במ"מ — היחידה שבה השרטוט מדבר ושבה המפעיל חושב.
// lgMmToMeterStr החזיר "2" על 2000 ו-"0.5" על 500, ואז מי שהקליד 200
// בכוונה של 2000 קיבל בחזרה "2".
//
// מתחת ל-300 מודפס דווקא כמטרים, ובכוונה: מספר שלם עד 300 נקרא בחזרה
// כס"מ, ולכן "250" היה הופך ל-2500 מ"מ ברינדור הבא — שקט לגמרי. הנקודה
// העשרונית היא מה שמחזיר אותו נכון. כל ערך חייב לשרוד את הדרך לשדה וחזרה.
function lgMmToInputStr(mm) {
  if (mm == null || mm === '') return '';
  const n = Math.round(Number(mm));
  if (!isFinite(n) || n <= 0) return '';
  return n > 300 ? String(n) : (n / 1000).toString();
}
